...
//            }
            ...
            com_data.rx_data[com_data.rx_data_write_pointer] = readBuffer[i];
            com_data.rx_data_write_pointer = com_data.rx_data_write_pointer + 1;
            if(com_data.rx_data_write_pointer >= MAX_COM_DATA_BUFFER_SIZE)
            {
                com_data.rx_data_write_pointer = 0;
            }
            com_data.phy_data_received = 1;
        }

        if(com_data.tx_data_available == 1)
        {
             ...
             * the "echo" data now.
             */
            for (i=0;i<com_data.tx_data_length;i++)
            {
                writeBuffer[i] = com_data.tx_data[i];
            }
            putUSBUSART(writeBuffer,com_data.tx_data_length);
        }
    }
